...
1. If the KeyDB server is password protected via the `requirepass` option.
2. If a KeyDB 6.0 instance, or greater, is using the [KeyDB ACL system](/docs/acl).

KeyDB versions prior of KeyDB 6 were only able to understand the one argument
version of the command:

    AUTH <password>

This form just authenticates against the password set with `requirepass`.
In this configuration KeyDB will deny any command executed by the just
...
When KeyDB ACLs are used, the command should be given in an extended way:

    AUTH <username> <password>

In order to authenticate the current connection with one of the connections
defined in the ACL list (see `ACL SETUSER`) and the official [ACL guide](/docs/acl) for more information.
...
Because of the high performance nature of KeyDB, it is possible to try
...
Instruct KeyDB to start an [Append Only File](https://docs.keydb.dev/docs/persistence/) rewrite process.
...
* If a KeyDB child is creating a snapshot on disk, the AOF rewrite is _scheduled_ but not started until the saving child producing the RDB file terminates. In this case the `BGREWRITEAOF` will still return an positive status reply, but with an appropriate message. You can check if an AOF rewrite is scheduled looking at the `INFO` command.
* If an AOF rewrite is already in progress the command returns an error and no
  AOF rewrite will be scheduled for a later time.i
...
The AOF rewrite is automatically triggered by KeyDB, however the
...
Since KeyDB 6.2, the recommended mode for client pause is `WRITE`. This mode will stop all replication traffic, can be
...
This command also prevents keys to be evicted or
...
mainly used to fix a cluster stuck in a wrong state by `keydb-cli --cluster fix`.
...
```GEOADD <key> <OPTIONAL:NX|XX> <OPTIONAL:CH> <longitude-1> <latitude-1> <name-1> ... <longitude-n> <latitude-n> <name-n>```
...
Since `GEORADIUS` and `GEORADIUSBYMEMBER` have a `STORE` and `STOREDIST` option they are technically flagged as writing commands in the KeyDB command table. For this reason read-only replicas will flag them, and KeyDB Cluster replicas will redirect them to the master instance even if the connection is in read-only mode (see the `READONLY` command of KeyDB Cluster).

Two read-only variants of the commands are available. They are exactly like the original commands but refuse the `STORE` and `STOREDIST` options. The two variants are called `GEORADIUS_RO` and `GEORADIUSBYMEMBER_RO`, and can safely be used in replicas.
...
As per KeyDB 6.2, GETSET is considered deprecated. Please prefer `SET` with `GET` parameter in new code.
...
HMSET is considered deprecated. Please prefer `HSET` in new code.
...
*   `errorstats`: KeyDB error statistics
...
*   `used_cpu_sys`: System CPU consumed by the KeyDB server, which is the sum of system CPU consumed by all threads of the server process (main thread and background threads)
*   `used_cpu_user`: User CPU consumed by the KeyDB server, which is the sum of user CPU consumed by all threads of the server process (main thread and background threads)
*   `used_cpu_sys_children`: System CPU consumed by the background processes
*   `used_cpu_user_children`: User CPU consumed by the background processes
*   `used_cpu_sys_main_thread`: System CPU consumed by the KeyDB server main thread
*   `used_cpu_user_main_thread`: User CPU consumed by the KeyDB server main thread
...
The **errorstats** section enables keeping track of the different errors that occurred within KeyDB, 
...
"OK"
keydb-cli>REPLICAOF 127.0.0.1 6799
"OK"
```
...
As per KeyDB 6.2.0, RPOPLPUSH is considered deprecated. Please prefer `LMOVE` in
new code.
