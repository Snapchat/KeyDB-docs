...
While it is possible for a KeyDB server S to be configured to become a replica of itself (by being issued `REPLICAOF <hostname-of-server-S> <port-of-server-S>`), doing so would create an additional and unneeded replication traffic for this server S. The added traffic will incur a performance penalty. 
