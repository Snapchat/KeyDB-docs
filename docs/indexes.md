---
id: indexes
title: Secondary Indexing with KeyDB
sidebar_label: Indexing
---


KeyDB is not exactly a key-value store, since values can be complex data structures. However it has an external key-value shell: at API level data is addressed by the key name. It is fair to say that, natively, KeyDB only offers *primary key access*. However since KeyDB is a data structures server, its capabilities can be used for indexing, in order to create secondary indexes of different kinds, including composite (multi-column) indexes.

This document explains how it is possible to create indexes in KeyDB using the following data structures:
...
Implementing and maintaining indexes with KeyDB is an advanced topic, so most
...
scenarios, there is the explicit need to store indexed data into KeyDB in order to speedup common queries which require some form of indexing in order to be executed.

Simple numerical indexes with sorted sets
===

The simplest secondary index you can create with KeyDB is by using the
...
For example I may have KeyDB hashes representing users. Each user is
...
so if the user `john` turned 39 years old, in order to update the
...
KeyDB sorted sets have an interesting property. When elements are added
...
This KeyDB feature is basically equivalent to a `b-tree` data structure which
is often used in order to implement indexes with traditional databases.
As you can guess, because of this, it is possible to use this KeyDB data
...
offer possible search keywords starting for "bit". We send KeyDB a command
like that:
...
Since lexicographical ranges in KeyDB are binary safe you can use any
...
the least significant bytes. This way when KeyDB compares the strings with
...
in order to optimize complex queries. In KeyDB they could be useful both
to implement a very fast in-memory KeyDB index of something stored into
a traditional data store, or in order to directly index KeyDB data.
...
different ways. For example, who are all the people `john`
*is friend of*?
...
Or, what are all the relationships `john` and `matteocollina` have where
...
the query in a very efficient way using KeyDB lexicographical ranges.
...
in the future may be implemented in KeyDB in a native way.
...
item. However other KeyDB data structures such as Sets or Lists can be used
...
Another popular index often used with KeyDB is a **capped list**, where items
...
Different strategies could be used. If the index data is outside KeyDB
*read repair* can be a solution, where data is fixed in a lazy way when
it is requested. When we index data which is stored in KeyDB itself
