---
id: ami-enterprise
title: Using the KeyDB Enterprise AMI
sidebar_label: KeyDB AWS AMI
---


## About the KeyDB Enterprise AMI

The KeyDB Enterprise AWS AMI is an Amazon Machine Image that is fully set up and configured for use. The AMI can be set up in seconds and launched through the Amazon interface, through the client, or API. You can also SSH into it and run the keydb-setup tool.
...
Benefits of the AMI:
* No need to purchase a license key. Enterprise is already enabled exclusive to this image. Just pay your hourly AWS costs.
...
You can also spin up a new AMI with the newer KeyDB Enterprise AMI as we update the AMIs with new point releases.

**What is the underlying distribution this image is build on?**
This is built on an Ubuntu18.04 64-bit x86 image. It uses a KeyDB Enterprise DEB package similar to our PPA repository Enterprise downloads. The services can be used the same.
...
### Select the KeyDB Enterprise AMI
...
It is recommended to configure via the user data script, unless you are familiar with linux and wish to set up KeyDB Enterprise manually. You will see a detailed breakdown for the script below
...
As new versions of KeyDB Enterprise are released they will be updated with new AMIs. However if you do not wish to update your AMI but just want to update the KeyDB binaries, you can run the following script:
```
...
With the KeyDB Enterprise AMI, you will have access to a basic level of KeyDB Support. Should you encounter an issue please reach out to support@keydb.dev. We not provide guarenteed response times with support for the AMI, however we will do our best to try and respond to your issues as soon as we can. Should you require a higher level of support with gurenteed response times please let us know and we will be happy to discuss a plan that will meet your needs.
