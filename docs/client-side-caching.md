...
title: KeyDB server-assisted client side caching
sidebar_label: Client side caching
---

# KeyDB server-assisted client side caching
...
patterns, when using KeyDB, leverage the Pub/Sub system in order to
...
For this reason KeyDB 6 implements direct support for client side caching,
in order to make this pattern much simpler to implement, more accessible,
reliable and efficient.

## The KeyDB implementation of client side caching

The KeyDB client side caching support is called _Tracking_, and has two modes:
...
server would end up storing too much information. For this reason KeyDB uses two
...
is a KeyDB array with just a single element. Since we send an array, if there
...
So far we described the first client side caching model that KeyDB implements.
...
## Limiting the amount of memory used by KeyDB

Just make sure to configure a suitable value for the maximum number of keys remembered by KeyDB, or alternatively use the BCAST mode that consumes no memory at all in the KeyDB side. Note that the memory consumed by KeyDB when BCAST is not used, is proportional both to the number of keys tracked, and the number of clients requesting such keys.
