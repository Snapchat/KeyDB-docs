---
id: clients
title: KeyDB Client Handling
sidebar_label: Client Handling
---


This document provides information about how KeyDB handles clients from the
...
KeyDB accepts clients connections on the configured listening TCP port and
on the Unix socket if enabled. When a new client connection is accepted
the following operations are performed:

* The client socket is put in non-blocking state since KeyDB uses multiplexing and non-blocking I/O.
* The `TCP_NODELAY` option is set in order to ensure that we don't have delays in our connection.
* A *readable* file event is created so that KeyDB is able to collect the client queries as soon as new data is available to be read on the socket.

After the client is initialized, KeyDB checks if we are already at the limit
of the number of clients that it is possible to handle simultaneously
(this is configured using the `maxclients` configuration directive.

In case it can't accept the current client because the maximum number of clients
was already accepted, KeyDB tries to send an error to the client in order to
...
closed immediately by KeyDB because the new socket output buffer is usually
...
However KeyDB does the following two things when serving clients:
...
32 (that is the number of file descriptors KeyDB reserves for internal uses),
then the number of maximum clients is modified by KeyDB to match the amount
...
When KeyDB is configured in order to handle a specific number of clients it
...
KeyDB needs to handle a variable-length output buffer for every client, since
...
to serve at a faster rate at which KeyDB can send the existing output to the
...
more and more memory. For this reason by default KeyDB sets limits to the
output buffer size for different kind of clients. When the limit is reached
the client connection is closed and the event logged in the KeyDB log file.

There are two kind of limits KeyDB uses:

* The **hard limit** is a fixed limit that when reached will make KeyDB closing the client connection as soon as possible.
...
By default recent versions of KeyDB don't close the connection with the client
...
You can configure this limit via `keydb.conf` or simply using `CONFIG SET timeout <value>`.
...
* Mission critical applications where a bug in the client software may saturate the KeyDB server with idle connections, causing service disruption.
...
In the above example session two clients are connected to the KeyDB server. The meaning of a few of the most interesting fields is the following:
...
The commands `CLIENT SETNAME` and `CLIENT GETNAME` can be used to set and get the connection name. Starting with KeyDB 4.0, the client name is shown in the
