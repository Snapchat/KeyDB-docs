...
client is served **in the middle** of the execution of a KeyDB
transaction. This guarantees that the commands are executed as a single
isolated operation.

* Either all of the commands or none are processed, so a KeyDB
...
are registered. KeyDB will detect this condition at restart, and will exit with an error. Using the `keydb-check-aof` tool it is possible to fix the
...
A KeyDB transaction is entered using the `MULTI` command. The command
...
When a KeyDB connection is in the context of a `MULTI` request,
...
commands in the queue are processed** – KeyDB will _not_ stop the
...
If you have a relational databases background, the fact that KeyDB commands
can fail during a transaction, but still KeyDB will execute the rest of the
...
of a key by 1 (let's suppose KeyDB doesn't have `INCR`).
