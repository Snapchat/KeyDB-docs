...
KeyDB keys are binary safe, this means that you can use any binary sequence as a
...
The KeyDB String type is the simplest type of value you can associate with
a KeyDB key. It is the only data type in Memcached, so it is also very natural
for newcomers to use it in KeyDB.

Since KeyDB keys are strings, when we use the string type as a value too,
...
Let's play a bit with the string type, using `keydb-cli` (all the examples
will be performed via `keydb-cli` in this tutorial).
...
Even if strings are the basic values of KeyDB, there are interesting operations
...
system that increments a KeyDB key using `INCR`
...
When `MGET` is used, KeyDB returns an array of values.
...
KeyDB expires: keys with limited time to live
---
...
called **KeyDB expires**. Basically you can set a timeout for a key, which
...
A few quick info about KeyDB expires:

* They can be set both using seconds or milliseconds precision.
* However the expire time resolution is always 1 millisecond.
* Information about expires are replicated and persisted on disk, the time virtually passes when your KeyDB server remains stopped (this means that KeyDB saves the date at which a key will expire).
...
can also create keys with expires using other KeyDB commands. For example
using `SET` options:
...
KeyDB lists are implemented via Linked Lists. This means that even if you have
...
KeyDB Lists are implemented with linked lists because for a database system it
is crucial to be able to add elements to a very long list in a very fast way.
Another strong advantage, as you'll see in a moment, is that KeyDB Lists can be
...
An important operation defined on KeyDB lists is the ability to *pop elements*.
...
KeyDB returned a NULL value to signal that there are no elements in the
list.
...
* Communication between processes, using a consumer-producer pattern where the producer pushes items into a list, and a consumer (usually a *worker*) consumes those items and executed actions. KeyDB has special list commands to make this use case both more reliable and efficient.

For example both the popular Ruby libraries [resque](https://github.com/resque/resque) and
[sidekiq](https://github.com/mperham/sidekiq) use KeyDB lists under the hood in order to
...
posted by users into KeyDB lists.
...
KeyDB allows us to use lists as a capped collection, only remembering the latest
...
The above `LTRIM` command tells KeyDB to take just list elements from index
...
1. Forces KeyDB and clients to process useless commands (all the requests when the list is empty will get no actual work done, they'll just return NULL).
2. Adds a delay to the processing of items, since after a worker receives a NULL, it waits some time. To make the delay smaller, we could wait less between calls to `RPOP`, with the effect of amplifying problem number 1, i.e. more useless calls to KeyDB.

So KeyDB implements commands called `BRPOP` and `BLPOP` which are versions
...
It is KeyDB' responsibility to delete keys when lists are left empty, or to create
an empty list if the key does not exist and we are trying to add elements
to it, for example, with `LPUSH`.

This is not specific to lists, it applies to all the KeyDB data types
...
KeyDB hashes look exactly how one might expect a "hash" to look, with field-value pairs:
...
KeyDB Sets are unordered collections of strings. The
...
Here I've added three elements to my set and told KeyDB to return all the
elements. As you can see they are not sorted -- KeyDB is free to return the
elements in any order at every call, since there is no contract with the
user about element ordering.

KeyDB has commands to test for membership. For example, checking if an element exists:
...
a KeyDB hash, which maps tag IDs to tag names.

There are other non trivial operations that are still easy to implement
using the right KeyDB commands. For instance we may want a list of all the
...
in the context of set theory, so the KeyDB command is called `SCARD`.
...
every time we add an element KeyDB performs an O(log(N)) operation. That's
good, but when we ask for sorted elements KeyDB does not have to do any work at
...
We asked KeyDB to return all the elements with a score between negative
...
which in the case of the KeyDB implementation is less than 1%.  The
...
HLLs in KeyDB, while technically a different data structure, are encoded
as a KeyDB string, so you can call `GET` to serialize a HLL, and `SET`
...
There are other important things in the KeyDB API that can't be explored
...
* KeyDB is also a Pub-Sub server.
...
Thanks for reading, and have fun hacking with KeyDB!
